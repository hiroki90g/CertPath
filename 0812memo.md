...
3. "New Project" でプロジェクト作成
4. Name: certpath, Region: Northeast Asia (Tokyo)
5. Database Password設定（強固なパスワード）
```
...
# .env.local ファイル作成（.gitignoreで除外済み）
```
...
// 接続確認用のテストページ実装
// http://localhost:3000/test-supabase でアクセス可能
```

### ⚠️ 重要な注意点

#### セキュリティ
- **環境変数**: `.env.local`は絶対にGitにコミットしない（.gitignore済み）
- **APIキー**: `ANON_KEY`はフロントエンド用（制限あり）、`SERVICE_ROLE_KEY`は使わない
- **RLS必須**: Row Level Security設定で適切なアクセス制御が必要

#### 接続テストのポイント
- **新規プロジェクト**: テーブルが存在しないエラーは正常
- **エラーコード**: `PGRST116`, `42883`は接続成功の証拠
- **認証エラー**: `PGRST301`, JWT関連は設定ミス

#### 開発時の注意
```bash
# 環境変数の再読み込み
npm run dev を再起動が必要（.env.local変更後）

# TypeScriptエラー回避
process.env.NEXT_PUBLIC_SUPABASE_URL # OK
supabase.supabaseUrl                 # NG (protected property)
```

### 次のステップ
1. **データベーステーブル作成**: SQL Editorで7つのテーブル作成
2. **RLS設定**: セキュリティポリシー設定
3. **Google OAuth**: 認証プロバイダー設定
4. **認証UI**: ログイン・サインアップページ実装